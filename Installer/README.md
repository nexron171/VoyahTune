...
Коды завершения: `0` — успех, `1` — ошибка операции/комплекта, `2` — неверные аргументы CLI, `130` — отмена. `apply` и `apply-plan` выводят JSONL с именем шага, исходным выводом и конкретной ошибкой. `--interactive` принимает `{"cancel":true}` через stdin; закрытие stdin или потеря stdout тоже запрашивает остановку на границе шага.
...
Для GUI: `npm --prefix Installer/desktop run dev`, затем `http://127.0.0.1:1420/?fixture=ready`. Дополнительные сценарии: none, multiple, unauthorized, root-error, events-error. Fixtures исключаются из production.
