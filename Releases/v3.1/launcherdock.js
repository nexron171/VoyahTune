...
// Хукаем ТОЛЬКО com.qinggan.launcher.navigation.NavigationBarMain (водительский/верхний экран).
// NavigationBarSecond (пассажир) и CN-класс NavigationBar НЕ трогаем.
//
...
Java.perform(function () {
    ...
    // Бэкап штатных фонов слотов: originalBg[viewName] = Drawable (один раз).
    var originalBg = {};
    ...
    function updateIcons(instance) {
        try {
            ...
            for (var name in map) {
                ...
                if (!view) continue;
                if (!originalBg[name]) originalBg[name] = view.getBackground();   // бэкап штатного фона один раз
                var pkg = map[name];
                if (pkg === "none" || !isInstalled(pkg)) {
                    view.setBackground(originalBg[name]);                          // восстановить штатную иконку
                } else {
