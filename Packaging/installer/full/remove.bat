...
adb.exe shell "ps -ef | grep frida-inject | grep -E 'vd_bypass|steeringwheelkeys|launcherdock|multidisplay|apollo_tech|keyboard_lock_en|keyboard_ru|fullscreen_client' | grep -v grep | awk '{print $2}' | xargs kill -9"
...
adb.exe shell "rm -f /data/local/bin/fullscreen_client.js /data/local/tmp/voyahtune_fullscreen_client.*"
...
adb.exe shell "test ! -e /data/local/bin/fullscreen_client.js && ! ls /data/local/tmp/voyahtune_fullscreen_client.* >/dev/null 2>&1"
if errorlevel 1 exit /b 1
